...
class ModulesListsAPI(Resource):
    def __init__(self):
      ...
      self.reqparse.add_argument('forAdmin', type=bool, help='This module is reserved for admin ?')
    ...
    def post(self):
      ...
      try:
        #get if module exist
        module = database.Modules.get(database.Modules.page == args['page'])

        return {'status' : 100, 'message' : 'This module already exist'}

      except DoesNotExist:
        module = database.Modules.create(type_id=args['type_id'], name=args['name'], page=args['page'], forAdmin=args['forAdmin'])

        return jsonify({'status' : 200,  'message' : 'Module successfully added', 'id' : module.id})
