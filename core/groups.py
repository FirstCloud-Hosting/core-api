...
class GroupsListsAPI(Resource):
    ...
    def get(self):
      ...
      query = (database.Groups.select()
              .where(database.Groups.organization_id  == user.organization_id)
              .order_by(database.Groups.name))
    ...
    def post(self):
      ...
      group = database.Groups.create(organization_id=user.organization_id, name=args['name'], note=args['note'])

      return jsonify({'status' : 200,  'message' : 'Group successfully added', 'id' : group.id})
...
class GroupAPI(Resource):
  ...
  def get(self, group_id):
    ...
    try:
      ...
      if len(query) != 0:
        ...
        query = database.Groups.get(database.Groups.id == group_id, database.Groups.organization_id == user.organization_id)
        return jsonify({'status' : 200,  'data' : model_to_dict(query)})

      else:

        query = database.Groups.get(database.Groups.id == user.group_id, database.Groups.organization_id == user.organization_id)
        return jsonify({'status' : 200,  'data' : model_to_dict(query)})
  ...
  def put(self, group_id):
    ...
    #update group
    query = database.Groups.update(name=args['name'], note=args['note']).where(database.Groups.id == group_id, database.Groups.organization_id == user.organization_id)
    query.execute()
  ...
  def delete(self, group_id):
    ...
    query = database.Permissions.delete().where(database.Permissions.group_id == group_id)
    query.execute()

    #delete group
    query = database.Groups.delete().where(database.Groups.id == group_id, database.Groups.organization_id == user.organization_id)
    query.execute()
