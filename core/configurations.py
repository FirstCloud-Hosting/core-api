...
class ConfigurationsAPI(Resource):
        def __init__(self):
            ...
            self.reqparse.add_argument('value', type=str, help="Value for edit configuration")
        ...
        def get(self, name):
            try:
                ...
                data = database.ApiConfigurations.get(database.ApiConfigurations.name == name)
        ...
        def put(self, name):
            ...
            query = database.ApiConfigurations.update({database.ApiConfigurations.value:args['value']}).where(database.ApiConfigurations.name == name)
            query.execute()
