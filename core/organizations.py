...
class OrganizationsAPI(Resource):
    ...
    def put(self, organization_id):
        ...
        query = database.ApiConfigurations.update(
            {
                database.Organizations.passwordHardening: args['passwordHardening']}).where(
            database.ApiConfigurations.id == organization_id)
        query.execute()
...
api.add_resource(
    OrganizationsAPI,
    '/1.0/organizations/<string:organization_id>')
